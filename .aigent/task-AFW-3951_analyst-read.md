...
Analyze Jira AFW-3951: [Rudderstack] Setup "Button Clicked" rudderstack event on webflow & react
...
2. Understand that coverage fully: entry URL/path, user role, fields and labels, buttons/CTAs, validation, success and error states, downstream events, test data, locales, and documented exclusions.
3. Extract verbatim facts testers will assert: copy, labels, URLs/paths, event names, thank-you text, error messages, property names. Quoted or backticked names are exact.
