...
- Read the named flows before writing cases: Work only from the flows or regression name you entered, plus Data Resources and project knowledge. Do not implement.
Read
...
Last synced: 2026-08-25T11:44:30.692Z
...
inScope must be atomic TESTABLE behaviours of the named flows, each with an observable expected result from knowledge. Do not invent. Do not list BASE_URL, environment, or unloaded sheets in ambiguous. Set blocked true only when knowledge has no observable expected result for the named flows.
